...
def upload_file(collection_name : Optional[str] = "test_collection"):
    ...
    if filename.endswith('.pdf'):
        data = load_split_pdf_file(os.path.join(app.config['UPLOAD_FOLDER'], filename))
    elif filename.endswith('.html'):
        data = load_split_html_file(os.path.join(app.config['UPLOAD_FOLDER'], filename))
    else:
        return {"message": "Only pdf and html files are supported"}
    
    db = create_vector_db(data, "test_vector_db_model_name", collection_name)
